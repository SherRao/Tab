...
- Email verification as a separate concept (the magic link *is* verification).
...
*Why not Auth.js/Lucia/Better-Auth*: none support SQLite+drizzle zero-config better than ~80 lines we control, and every one pulls adapter layers we don't need. Magic link + session is small enough to own. *Alternative rejected*: signing JWTs directly (no server-side revocation; sign-out wouldn't actually kill sessions).
...
*Why not silent claim*: bare-name guests carry no credential; the owner is the only trust anchor who can vouch "Bob is this Bob". Decided during exploration.
